-- ---- o título do lead deixa de guardar telefone (fork Salvador Broker) ----
--
-- ⚠️ AQUI E NÃO NA CADEIA DE MIGRATIONS, pela regra desta casa: migration `9xxx`
-- cria objeto NOSSO; o que mexe em objeto do upstream vive em `supabase/local`.
-- Este bloco faz `update` em `public.crm_leads`, que é dele.
--
-- É idempotente por construção: só reescreve a linha cujo `title` é EXATAMENTE
-- igual ao telefone do contato. Rodar de novo não acha nada e não custa nada,
-- que é o contrato do `aplicar_sql_local` (install.sh, update.sh e os dois rigs
-- de teste o aplicam a cada passagem).
--
-- O conserto que IMPEDE o telefone de voltar a ser gravado está no código:
-- `lib/leads/nascimento-do-lead.ts`, `lib/automation/actions/create-or-move-lead.ts`
-- e `lib/webhooks/respondi.ts`. Este arquivo cuida do que já estava gravado.
