...
import { ehIdentificadorTecnico, nomeDoContato, SEM_NOME } from "@/lib/contacts/rotulo-do-contato";
...
export async function garantirLeadDaConversa(
  ...
  // Por isso usamos `nomeDoContato` (o upstream extraiu a mesma metade "nome" por conta própria,
  // no #906, e é dele que ela vem agora) — a metade sem o fallback de
  ...
  const doCadastro = nomeDoContato(contato) ?? SEM_NOME;
