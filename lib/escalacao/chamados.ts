...
export async function listarChamados(
  ...
  organizationId: string,
  opts: { estado: "abertos" | "fechados"; limite?: number; visiveisPara: ConversasVisiveis; actor: Actor },
  ...
  return {
    chamados: ((data ?? []) as unknown as LinhaComContato[]).map((r) => achatarContato(r, opts.actor)),
    abertos: count ?? 0,
  };
}
...
export async function lerChamado(
  ...
  organizationId: string,
  caseId: string,
  opts: { visiveisPara: ConversasVisiveis; actor: Actor },
  ...
  return {
    ...achatarContato(linha, opts.actor),
