...
/**
 * ⚠️ `visiveisPara: "todas"` nos casos abaixo, e isso não afrouxa nada aqui: o
 * recorte por atendente é do UPSTREAM (v1.41.0) e mede outra coisa — quais
 * CONVERSAS o papel enxerga. O que este arquivo mede é o recorte do DADO
 * sensível, por quem cadastrou o contato. Fixar o primeiro em "todas" isola o
 * segundo, que é o objeto do teste; se os dois variassem juntos, um verde aqui
 * não diria qual dos dois protegeu.
 */
...
describe("listarChamados — proteção de telefone do contato", () => {
  it("agent que não cadastrou o lead recebe contact_phone nulo e contact_protected=true", async () => {
    ...
    const { chamados } = await listarChamados(supabase, ORG, {
      estado: "abertos",
      visiveisPara: "todas",
      actor: AGENT_QUE_NAO_CADASTROU,
    });
  ...
  it("agent que cadastrou o próprio lead vê o telefone normalmente", async () => {
    const supabase = fakeSupabase([chamadoRow()]);
    const { chamados } = await listarChamados(supabase, ORG, { estado: "abertos", visiveisPara: "todas", actor: AGENT_CRIADOR });
  ...
  it("manager vê o telefone mesmo sem ter cadastrado o lead", async () => {
    const supabase = fakeSupabase([chamadoRow()]);
    const { chamados } = await listarChamados(supabase, ORG, { estado: "abertos", visiveisPara: "todas", actor: MANAGER });
    expect(chamados[0]?.contact_phone).toBe("+5531988887777");
    expect(chamados[0]?.contact_protected).toBe(false);
  });
});
...
describe("lerChamado — proteção de telefone do contato", () => {
  it("agent que não cadastrou o lead recebe o detalhe do caso com telefone nulo", async () => {
    const supabase = fakeSupabase([chamadoRow()]);
    const chamado = await lerChamado(supabase, ORG, CASE_ID, { visiveisPara: "todas", actor: AGENT_QUE_NAO_CADASTROU });
