 ...
 *
 * ⚠️ EXCETO quando `tetoMs` é passado, que é o que esta casa faz (issue #196,
 * reportada por @Gervanno medindo em produção: 3 bolhas em ~30s viraram 3
 * respostas em menos de 2 minutos). Aí a carona ESTENDE a janela — debounce
 * passa a ser espera de SILÊNCIO depois da última mensagem — até o teto contado
 * desde a primeira. Sem `tetoMs` o módulo é exatamente o do upstream, e os
 * testes dele continuam medindo esse ramo.
 */
...
/**
 * A carona desta casa: acha o job E estende a janela no MESMO statement, o que
 * fecha de graça a corrida entre duas mensagens quase simultâneas que um
 * SELECT-depois-UPDATE deixaria aberta.
 *
 * `least(...)` aplica o teto contado desde `created_at` (a primeira mensagem da
 * rajada) e `greatest(run_after, ...)` garante que estender nunca vire
 * ANTECIPAR: se um backoff já empurrou o job para mais longe, ele fica onde
 * está. A exclusão do job em HOLD é a MESMA de `SQL_JOB_PARA_COALESCER`; sem
 * ela este UPDATE seria pior que o select, porque ESCREVERIA num job morto.
 */
...
/** Carona com janela deslizante até `tetoMs` desde a primeira mensagem. */
export async function estenderJanelaDoJob(
  pool: pg.Pool,
  alvo: { organizationId: string; contactId: string },
  debounceMs: number,
  tetoMs: number,
 ...
 *
 * `tetoMs` liga a janela deslizante desta casa (ver o cabeçalho).
 */
  ...
  if (debounceMs > 0) {
    const jobId =
      tetoMs === undefined
        ...
        : await estenderJanelaDoJob(pool, alvo, debounceMs, tetoMs);
